...
public class FileDao {
	...
	public FileDto getData(int num) {
		...
		try {
			conn = new DbcpBean().getConn();
			//실행할 sql 문 작성
			String sql = "SELECT writer, title, orgFileName, saveFileName, downCount, fileSize, regdate FROM board_file WHERE num=?";
			pstmt = conn.prepareStatement(sql);
			//? 에 값 바인딩
			pstmt.setInt(1, num);
	...
	public boolean insert(FileDto dto) {
		...
		try {
			conn = new DbcpBean().getConn();
			//실행할 sql 문 작성하기
			String sql = "INSERT INTO board_file (num, writer, title, orgFileName, saveFileName, fileSize, regdate) "
